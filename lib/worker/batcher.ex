...
defmodule BorsNG.Worker.Batcher do
  ...
  defp maybe_complete_batch(batch) do
    ...
    status_from_db = Batcher.State.summary_database_statuses(statuses)
    now = DateTime.to_unix(DateTime.utc_now(), :second)
    batch_status = if status_from_db != :running do
      # need to change status (could go from :ok to :error if non-ff push)
      completed_status = complete_batch(status_from_db, batch, statuses)

      # status can change so send_status should come after complete_batch
      batch.project
      |> get_repo_conn()
      |> send_status(batch, status_from_db)
      Project.ping!(batch.project_id)
    ...
    batch
    |> Batch.changeset(%{state: batch_status, last_polled: now})
    |> Repo.update!()
    if batch_status != :running do
